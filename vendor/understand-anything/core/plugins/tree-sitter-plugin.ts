...
import type { LanguageExtractor } from "./extractors/types";
import { builtinExtractors } from "./extractors/index";
...
export class TreeSitterPlugin implements AnalyzerPlugin {
   ...
   * unchanged.
   */
  private wasmResolver?: (wasmPackage: string, wasmFile: string) => string;
  ...
  constructor(
    ...
    wasmResolver?: (wasmPackage: string, wasmFile: string) => string,
  ) {
   ...
   */
  private resolveWasm(wasmPackage: string, wasmFile: string): string {
