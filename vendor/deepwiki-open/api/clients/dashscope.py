...
class DashscopeClient(ModelClient):
    ...
    def parse_chat_completion(
        ...
        try:
            ...
            if hasattr(completion, "choices") and hasattr(completion, "usage"):
                ...
                return GeneratorOutput(
                    data=full_content, usage=usage, raw_response="streaming"
                )
        except Exception as e:
        ...
        try:
            ...
            if result.data:
                ...
                if len(result.data) > 0:
                    log.info(
                        f"🔍 First embedding length: {len(result.data[0].embedding) if hasattr(result.data[0], 'embedding') else 'N/A'}"
                    )
            else:
                log.warning("🔍 No embedding data found in result")
            return result
        except Exception as e:
    ...
    def convert_inputs_to_api_kwargs(
        ...
        if model_type == ModelType.LLM:
            ...
            api_kwargs = {"input": processed_input, **final_model_kwargs}
    ...
    def call(self, api_kwargs: Dict = {}, model_type: ModelType = ModelType.UNDEFINED):
        ...
        if model_type == ModelType.LLM:
            ...
            log.info(
                f"🔍 DashScope embedding API call with {len(valid_texts)} valid texts out of {len(texts)} total"
            )

            try:
                response = self.sync_client.embeddings.create(**filtered_api_kwargs)
                log.info(
                    f"🔍 DashScope API call successful, response type: {type(response)}"
                )
                ...
                if len(valid_texts) != len(texts):
                    ...
                    embedding_dim = (
                        None  # Must be determined from a successful response
                    )
                    if (
                        result.data
...
class DashScopeBatchEmbedder(DataComponent):
        ...
        if not force_recreate and os.path.exists(self.cache_path):
            try:
                with open(self.cache_path, "rb") as f:
                    ...
                    log.info(
                        f"Loaded cached DashScope embeddings from: {self.cache_path}"
                    )
                return embeddings
            except Exception as e:
                log.warning(
                    f"Failed to load cache file {self.cache_path}: {e}, proceeding with fresh embedding"
                )
    ...
    def __call__(
        ...
        """
        return self.call(
            input=input, model_kwargs=model_kwargs, force_recreate=force_recreate
        )
