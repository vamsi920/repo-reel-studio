...
// had: its Escape handler lived on a plain `role="presentation"` div that sits
// beside the dialog content in the DOM, not above it, so a keydown bubbling up
// from a focused control inside the dialog never reached it — Escape did
// nothing — and neither Tab-trapping nor initial focus were wired up at all.
// These tests exercise the dialog through `ModalBackdrop` the same way a
// keyboard user would.
describe("TurnOffConfirmationModal", () => {
  ...
  it("calls onConfirm when Turn Off is clicked", async () => {
    const user = userEvent.setup();
