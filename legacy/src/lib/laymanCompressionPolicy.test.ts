...
describe("laymanCompressionPolicy", () => {
  ...
  it("rejects contexts not in allow list", () => {
    configureLaymanCompressionPolicy({ allowContexts: ["video_prose_context"] });
  ...
  it("records instrumentation with context and token counts", () => {
    ...
    expect(report.byContext.video_prose_context.savedTokens).toBeGreaterThanOrEqual(0);
  });
  ...
  it("never creates .original backup files in app integration", () => {
    ...
    writeFileSync(promptPath, "This is basically prompt prose for compression.");
