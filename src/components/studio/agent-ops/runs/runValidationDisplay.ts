...
export function validationOverallTone(status: AgentRunValidation["overallStatus"]) {
  if (status === "passed") return "text-emerald-100 border-emerald-300/25 bg-emerald-300/8";
  if (status === "partial") return "text-amber-100 border-amber-300/25 bg-amber-300/8";
  if (status === "failed") return "text-rose-100 border-rose-300/25 bg-rose-300/8";
