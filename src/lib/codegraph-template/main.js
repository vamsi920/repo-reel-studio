const defaultCollapsedPanels = new Set([
    'legend-panel',
    'massive-objects',
    'size-toggle-panel',
]);
...
document.querySelectorAll('.panel').forEach(panel => {
    ...
    if (defaultCollapsedPanels.has(panel.id)) {
...
const dependencyLinks = graphData.links.filter(l => l.type === 'dependency').length;
document.getElementById('stats-content').innerHTML = `
    <div class="stats-grid">
        <div class="stats-card">
            <div class="stats-card-label">Modules</div>
            <div class="stats-card-value">${moduleCount}</div>
        </div>
        <div class="stats-card">
            <div class="stats-card-label">Entities</div>
            <div class="stats-card-value">${entityCount}</div>
        </div>
        <div class="stats-card">
            <div class="stats-card-label">Paths</div>
            <div class="stats-card-value">${moduleLinks}</div>
        </div>
        <div class="stats-card">
            <div class="stats-card-label">Calls</div>
            <div class="stats-card-value">${dependencyLinks}</div>
        </div>
    </div>
    <div class="stats-caption">Search any node, zoom to focus, and drag modules to compare architecture paths.</div>
`;
...
if (unlinkedModules.length > 0) {
    ...
    document.getElementById('unlinked-list').innerHTML = '<div class="empty-state">No isolated modules in this map.</div>';
}
...
function updateLinksCount() {
    ...
    if (nodesWithLinks.length > 0) {
        ...
        document.getElementById('links-count-content').innerHTML = '<div class="empty-state">No nodes match the current filter.</div>';
    }
}
...
function formatNodeLabel(node) {
    const raw = node.label || node.id || 'node';
    const limit = node.type === 'module' ? 20 : 16;
...
function updateMassiveObjects() {
    ...
    document.getElementById('massive-list').innerHTML = massiveNodes.length > 0
        ? massiveNodes.map(n => `
            <li data-node-id="${n.id}" title="${n.type === 'module' ? n.fullPath : n.parent}">
        ...
        `).join('')
        : '<div class="empty-state">No large files or symbols above the current threshold.</div>';
...
const scaleFactor = nodeCount > 60 ? 1 + (nodeCount - 60) / 80 : 1;

// Create force simulation with adjusted parameters for better spacing
const simulation = d3.forceSimulation(graphData.nodes)
    .force("link", d3.forceLink(graphData.links).id(d => d.id).distance(d => {
        const base = d.type === 'module-module' ? 240 : d.type === 'module-entity' ? 88 : 110;
        return base * scaleFactor;
    }).strength(d => d.type === 'module-module' ? 0.32 / scaleFactor : 0.38 / scaleFactor))
    .force("charge", d3.forceManyBody().strength(d => {
        const base = d.type === 'module' ? -680 : -240;
        return base * scaleFactor;
    }))
    .force("center", d3.forceCenter(width / 2, height / 2).strength(0.06 / scaleFactor))
    .force("x", d3.forceX(width / 2).strength(0.03 / scaleFactor))
    .force("y", d3.forceY(height / 2).strength(0.03 / scaleFactor))
    .force("collision", d3.forceCollide().radius(d => {
        const base = d.type === 'module' ? 56 : 28;
        return base * scaleFactor;
    }).strength(1));
...
node.each(function(d) {
    ...
    if (d.type === "module") {
        ...
        const accentWidth = Math.max(26, size * 0.68);
        el.append("rect")
            .attr("class", "node-module-shell node-shape")
            .attr("width", shellSize)
            .attr("height", shellSize)
            .attr("x", -shellSize / 2)
            .attr("y", -shellSize / 2)
            .attr("rx", 14);
        el.append("rect")
            .attr("class", "node-module node-shape")
            .attr("width", size)
            .attr("height", size)
            .attr("x", -size / 2)
            .attr("y", -size / 2)
            .attr("rx", 12);
        el.append("rect")
            .attr("class", "node-module-accent node-shape")
        ...
        el.append("circle")
            .attr("class", "node-entity-halo node-shape")
            .attr("r", r + 7);
        el.append("circle")
            .attr("class", "node-entity-ring node-shape")
            .attr("r", r + 2);
        el.append("circle")
        ...
        el.append("circle")
            ...
            .attr("r", 9);
        el.append("circle")
            .attr("class", "node-external node-shape")
            .attr("r", 6);
    }
});
...
function updateNodeSizes() {
    node.each(function(d) {
        ...
        if (d.type === "module") {
            ...
            const shellSize = size + 12;
            const accentWidth = Math.max(26, size * 0.68);
            el.select(".node-module-shell")
                .attr("width", shellSize)
                .attr("height", shellSize)
                .attr("x", -shellSize / 2)
                .attr("y", -shellSize / 2);
            el.select(".node-module")
                .attr("width", size)
                .attr("height", size)
                .attr("x", -size / 2)
                .attr("y", -size / 2);
            el.select(".node-module-accent")
                .attr("width", accentWidth)
                .attr("x", -accentWidth / 2)
                .attr("y", -size / 2 + 7);
        } else if (d.type === "entity") {
            const r = getNodeSize(d, 8);
            el.select(".node-entity-halo").attr("r", r + 7);
            el.select(".node-entity-ring").attr("r", r + 2);
            el.select(".node-entity").attr("r", r);
        }
    });
    // Update labels position
    labels.attr("dy", d => {
        if (d.type === "module") {
            return getNodeSize(d, 26) / 2 + 16;
        }
        return getNodeSize(d, 8) + 12;
    });
}
...
document.querySelectorAll('#unlinked-list li').forEach(li => {
    li.addEventListener('click', () => {
        ...
        if (targetNode) {
            ...
            node.selectAll(".node-shape")
                .style("filter", n => n.id === moduleId ? "brightness(1.2) drop-shadow(0 0 18px rgba(180, 197, 255, 0.4))" : "none");

            // Reset highlight after 2 seconds
            setTimeout(() => {
                node.selectAll(".node-shape").style("filter", null);
            }, 2000);
        }
    });
});
