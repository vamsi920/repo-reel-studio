// Vendored from Layman (https://github.com/vamsi920/layman), MIT licensed,
// Copyright (c) 2026 Julius Brussee. Upstream commit
// 76ec6347a09e6480c8a23a6c8879729ffdc13a43. This is the deterministic
// TypeScript port of `layman-compress/scripts/{compress,detect,validate}.py`.
// See THIRD_PARTY_NOTICES.md. Do not add compression rules here — the
// protected-token pass is what guarantees code, paths, URLs, commands and
// error strings survive compression byte-for-byte.
...
export type LaymanValidationWarningCode =
  | "path_mismatch"
...
const NATURAL_LANGUAGE_EXTENSIONS = new Set([
  ".md",
  ".markdown",
  ".txt",
  ".rst",
...
const LINE_NUMBER_REGEX =
  /(?:\bline\s+\d+\b|\bL\d{1,6}\b|:\d{1,6}(?::\d{1,6})?)/gi;
...
function applyPhraseReplacers(
  text: string,
  replacers: Array<[RegExp, string]>,
): string {
...
function looksMinified(text: string): boolean {
  ...
  const hasLowWhitespace =
    sample.length > 0 &&
    (sample.match(/\s/g)?.length ?? 0) / sample.length < 0.08;
  return (
    longLines / nonEmpty.length > 0.6 && hasManySemicolons && hasLowWhitespace
  );
}
...
export function detectCompressionContentKind(
  path: string,
  content: string,
): LaymanContentKind {
...
function refusalFromSkipReason(
  ...
  if (reason === "empty_input") {
    return {
      code: "empty_input",
      message: "Refusing compression: input is empty.",
    };
  }
  if (reason === "already_backup_file") {
    return {
      ...
      message:
        "Refusing compression: .original.md backup files are never compressed.",
    };
  }
  ...
  return {
    ...
    message:
      "Refusing compression: file classified as non-compressible (code/config/binary/generated).",
  };
}
...
export function evaluateCompressionEligibility(params: {
  ...
  if (!content.trim()) {
    ...
    return {
      ...
      reason,
      refusal: refusalFromSkipReason(reason),
    };
  }
  ...
  if (size > maxBytes) {
    ...
    return {
      shouldCompress: false,
      reason,
      refusal: refusalFromSkipReason(reason, { maxBytes }),
    };
  }
  ...
  if (sensitive.matched) {
    ...
    return {
      ...
      reason,
      refusal: refusalFromSkipReason(reason, {
        sensitiveCode: sensitive.reason ?? undefined,
      }),
    };
  }
  ...
  for (let i = 0; i < lines.length; ) {
    ...
    while (i < lines.length) {
      ...
      if (
        ...
        (maybeClose[2]?.length ?? 0) >= fenceLen &&
        maybeClose[3]?.trim() === ""
      ) {
        i += 1;
        break;
...
export function protectProseForLaymanCompression(
  text: string,
): LaymanProtectedProseSegment {
...
function estimatePotentialSavingsTokens(
  text: string,
  mode: LaymanBriefMode,
): number {
  ...
  const countPhrase = (phrase: string) =>
    normalized.split(phrase.toLowerCase()).length - 1;
...
export function estimateTokenMetrics(
  beforeText: string,
  afterText: string,
...
export function buildLaymanCompressionPayload(params: {
  ...
  payload: LaymanCompressionPayload | null;
  refusal: LaymanCompressionRefusal | null;
} {
  ...
  for (const segment of segments) {
    ...
    output.push(
      restoreProtectedLaymanTokens(
        simplified,
        protectedSegment.protectedTokens,
      ),
    );
  }
  return output.join("\n");
...
export function validateCompressedMarkdown(
  ...
  if (
    ...
    JSON.stringify([...compressedUrls].sort())
  ) {
    errors.push({
      code: "url_mismatch",
  ...
  if (
    JSON.stringify(originalInlineCode) !== JSON.stringify(compressedInlineCode)
  ) {
    errors.push({
  ...
  if (
    JSON.stringify(originalTableShapes) !==
    JSON.stringify(compressedTableShapes)
  ) {
    errors.push({
  ...
  if (
    JSON.stringify([...originalPaths].sort()) !==
    JSON.stringify([...compressedPaths].sort())
  ) {
    warnings.push({
      code: "path_mismatch",
  ...
  if (originalBulletCount > 0) {
    ...
    const diff =
      Math.abs(originalBulletCount - compressedBulletCount) /
      originalBulletCount;
    if (diff > 0.15) {
      warnings.push({
...
export function compressMarkdownProseOnlyWithValidation(
  ...
  const compressed = compressMarkdownProseOnly(
    markdown,
    simplifyProse,
    options,
  );
...
export function compressForPrompt(
  ...
  if (!eligibility.shouldCompress) {
    ...
    return {
      ...
      fallbackReason,
      mode,
      metrics: summarizeTokenEstimates({
        mode,
        originalText: text,
        compressedText: text,
      }),
  ...
  if (
    !options.simplifyProse &&
    estimatePotentialSavingsTokens(text, mode) === 0
  ) {
    return {
      text,
      usedCompression: false,
