 ...
 */
const LAYMAN_PROMPT_ENABLED_DEFAULT = true;
...
const DEFAULT_POLICY: LaymanCompressionPolicy = {
  enabled: LAYMAN_PROMPT_ENABLED_DEFAULT,
  mode: "full",
  minSavedTokens: 2,
...
function recordInstrumentationEvent(
  event: LaymanCompressionInstrumentationEvent,
): void {
...
export function getLaymanCompressionInstrumentationReport(): LaymanCompressionInstrumentationReport {
  const base: LaymanCompressionInstrumentationReport["byContext"] = {
    ...
    workspace_memory_context: { events: 0, savedTokens: 0, skipped: 0 },
  };
  ...
  return {
    ...
    totalSavedTokens: instrumentationEvents.reduce(
...
export function getLaymanCompressionPolicy(): LaymanCompressionPolicy {
  return {
    ...activePolicy,
    allowContexts: [...activePolicy.allowContexts],
    denyContexts: [...activePolicy.denyContexts],
  };
}
...
export function configureLaymanCompressionPolicy(
  ...
  activePolicy = {
    ...activePolicy,
    ...updates,
    allowContexts: updates.allowContexts
      ? [...updates.allowContexts]
      : [...activePolicy.allowContexts],
    denyContexts: updates.denyContexts
      ? [...updates.denyContexts]
      : [...activePolicy.denyContexts],
  };
...
export function resetLaymanCompressionPolicy(): LaymanCompressionPolicy {
  activePolicy = {
    ...DEFAULT_POLICY,
    allowContexts: [...DEFAULT_POLICY.allowContexts],
    denyContexts: [],
  };
  return getLaymanCompressionPolicy();
}

export function evaluateLaymanPolicy(
  context: LaymanCompressionContext,
  ...
  if (
    policy.allowContexts.length > 0 &&
    !policy.allowContexts.includes(context)
  ) {
...
export function compressForPromptWithPolicy(params: {
  ...
  if (!decision.allowed) {
    const blocked: LaymanPromptCompressionResult & {
      policyDecision: LaymanPolicyDecision;
    } = {
      text: params.text,
      usedCompression: false,
