...
describe("layman benchmark", () => {
  it("reports token savings, skipped counts, validation failures, and invariants", () => {
    ...
    const summary = {
      ...
      skippedCount,
      validationFailures,
      preservedInvariantChecks: FIXTURES.reduce(
        (sum, fixture) => sum + fixture.mustPreserve.length,
        0,
      ),
    ...
    console.info(
      JSON.stringify({ event: "layman_benchmark", summary }, null, 2),
    );
