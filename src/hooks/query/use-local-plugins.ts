 ...
 * the Plugins page. Mirrors `usePlugins`. Keyed by the active backend's id so
 * switching backends fetches fresh data instead of serving a stale list.
 */
export const useLocalPlugins = () => {
  const { backend } = useActiveBackend();
  return useQuery<LocalPlugin[]>({
    queryKey: PLUGINS_QUERY_KEYS.local(backend.id),
